...
export class SearchDebuggerView {
        ...
        switch (message.command) {
            ...
            case 'stateSelected':
                try {
                    ...
                    this.sendRequestToPlanimationApi(message.stateInfo, message.domain, message.problem, message.animation, message.stateId);
                }
                catch (ex) {
    ...
    sendRequestToPlanimationApi(subGoal: string, domain: string, problem: string, ap: string, id: number): void{
        const re = /(.*\(:goal\s*\(and\s*)((.|\n)*)(.*\)\s*\)\s*\))/;
        console.log(subGoal);
        var fd = new FormData();
        if (id == 0) {
            fd.append("problem", problem);
            fd.append("fileType", "fpng");
        } else {
            const newProblem = problem.replace(re, "$1" + subGoal + "$4");
            console.log(newProblem);
            fd.append("problem", newProblem);
            fd.append("fileType", "lpng");
        }
